%% MATH TOOLS 2019 LAB 2: MATLAB Intro II Answers
...
curr_num = randi([-50,50])
if curr_num >= -10 && curr_num <= 10
    curr_num = curr_num
elseif curr_num < -10
    curr_num = -10
elseif curr_num > 10
...
% sample2 rather than added. Uncomment this block of code and run it.
% What happens?
...
while num_ortho < 10
    % Draw a 2D row vector randomly from a normal distribution (hint:
    % use randn())
    A = randn(1,2);
    ...
    % Test if the random vectors are orthogonal. If so, increase the
    % counter for number of orthogonal vectors, and store each of the two
    % vectors in the corresponding index of their array.
    inner_prod = A*B;
    if abs(inner_prod) < 0.00001        % we use this rather than "== 0" because matlab has some numeric
        num_ortho = num_ortho + 1;      % variability (rounding), so we accept inner products very close to 0
        row_container(num_ortho,:)=A;
        column_container(:,num_ortho)=B;
    end
end
